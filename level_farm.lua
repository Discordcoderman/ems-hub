-- level_farm.lua
-- LevelFarm task: Refresh + Start. ManualLevelLookup table for Sea 1/2/3.
-- Uses Spirit.HoverOver for attack positioning (not circular orbit).
-- Depends on: core, data, tween, combat, tasks.
...
-- Direct table traversal — no dependency on ReplicatedStorage.Quests.
-- ═══════════════════════════════════════════════════════════════
...
LF:RegisterMethod("Start", function(step)
    ...
    -- Bailout: maxed players in Sea 1 don't need to farm here
    ...
    -- ── Step 4 / 1: default → ManualLevelLookup ────────────────
    else
        local Q = ManualLevelLookup()
        if not Q then
            Spirit.Report("LevelFarm: ManualLevelLookup returned nil (lv="
                .. tostring(currentLevel) .. ", sea=" .. tostring(Spirit.SeaIndex) .. ")")
            return
        end
        Spirit.SetTask("SubTask", Q.NameMon .. " | Quest: " .. Q.Mon)

        local activeQuest = Spirit.GetActiveQuestName()
        if activeQuest ~= nil then
            if activeQuest ~= false then
                if activeQuest ~= Q.Qname then
                    return Spirit.J.AbandonQuest(Spirit.J)
                end
            else
                if not Q.PosQ then return end
                Spirit.TweenController.Create(Q.PosQ + Vector3.new(0, 5, 3))
                Spirit.SetTask("MainTask", "Level Farm | " .. Q.Mon .. " | Accept Quest")
            ...
            -- GUI-text fallback (for when QuestUpdate remote isn't caught)
            local claim, _ = Spirit.GetCurrentClaimQuest()
            if claim then
                if claim ~= Q.NameMon and claim ~= (Q.NameMon .. "s") then
                    return Spirit.J.AbandonQuest(Spirit.J)
                end
            else
                if not Q.PosQ then return end
